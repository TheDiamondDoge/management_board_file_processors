...
public class RisksFileGenerator {
    ...
    public String generateXlsxFile(List<Risk> risks, String projectName) throws IOException, NoSheetFoundException {
        templateFile = getClass().getClassLoader().getResourceAsStream(TEMPLATE_RESOURCE);
        if (Objects.isNull(templateFile)) {
            throw new FileNotFoundException("Risks template not found");
        }
